...
import { getAllUsersSentRequest } from "@/utils/functions/getAllUsersSentRequest";
...
const page = () => {
  ...
  const getAllUserForSentRequest = async () => {
    setLoding(true);
    await getAllUsersSentRequest(setUsers,setError,route);
    setLoding(false);
}
  ...
  const handleSendRequest = async(to) => {
    await sendRequest(to, setError, toast, route);
      ...
      <div className="w-full sm:w-[400px] flex flex-col h-screen shadow-lg shadow-primary">
        ...
        <div className="w-full flex-1 pt-7 overflow-y-scroll">
          ...
          {loding === false && users.length === 0 && (
            <h1 className="text-2xl font-semibold text-center">No any Users</h1>
          )}
        </div>
      </div>

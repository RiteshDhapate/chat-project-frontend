...
import { getAllFrends } from "@/utils/functions/getAllFrends";
...
const page = () => {
  ...
  const getAllUserForSentRequest = async () => {
    setLoding(true);
    await getAllFrends(setUsers, setError, route);
    setLoding(false);
  };
  ...
  const handleRedirectChat = (id) => {
    route.push(`chats/${id}?type=friend`);
  };
          ...
          {loding === false &&
            ...
            searchUsers.map((user, index) => (
              <div
                key={index}
                onClick={() => handleRedirectChat(user._id)}
                className="w-full h-14 mt-5 bg-primar cursor-default flex justify-between items-center px-5 overflow-hidden hover:bg-gray-100 dark:hover:bg-black/10 "
              >
                <div className="flex">
                  <Image
                    src={user.profileImage}
